...
export default function GetWalletERC1155NFTs() {
  ...
  async function handleBurnNft(token_id, qty, emailaddress) {
    ...
    if (
      ...
      setShowModal(false);
      setBurnLoading(true);
      await (await marketPlaceInstance()).get(`/verifyEmail/${emailaddress}`)
        .then((data) => {
        ...
        .then(async () => {
          if (usernameConfirmed) {
            try {
              const result = await contract.erc1155.burnFrom(
                address,
                token_id,
                qty
              );
              ...
              await (await marketPlaceInstance()).get(`redeemTokenNFT/${address}/${token_id}/${user_id}/${qty}`
              ).then((data) => {
                //setNFTBalance(data.data);
                setBurnLoading(false);
                notify(
                  "You have successfully redeemed your NFT and " +
                    data.data +
  ...
  return (
    <div className='bordered-section'>
      ...
      <div className='pageTitless text-animate'>
        <h4>My Scrooge Casino Token NFTs</h4>
      </div>
