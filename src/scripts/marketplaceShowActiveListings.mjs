  ...
  return (
    <div>
      ...
      {loadingListings ? (
        ...
        <div className='flex-row erc-card-box'>
          {listings.map((listing) => (
            <div className='erc1155Card' key={listing.id}>
              ...
              <div className='erc1155Card-details'>
                {listing.asset.name.toString()}
                <br></br>
                <span className='erc1155-price'>
                  ${(listing.buyoutPrice / 10 ** 18).toFixed(2).toString()} BUSD
                </span>
                <br></br>
                <button
                  className='erc1155-buy-btn'
                  onClick={() => handleBuyAsset(listing.id, 1)}
                  id={listing.asset.name.toString()}
                >
                  BUY NFT!
                </button>
              </div>
