...
export default function SharableData() {
  ...
  const shortLink = (link) => {
    ...
    try {
      Axios.get(`https://34.237.237.45:9001/api/getShortLink/${encoded}`).then(
        (data) => {
  ...
  const createAffUser = async (user_id) => {
    ...
    try {
      const createdAffUser = await createAffiliateUser(user_id);
      console.log('cau', createdAffUser);
      if (createAffUser !== 'Error') {
        const aff = await getAffiliateUser(user_id);
            ...
            <div
              ...
              <div className='affiliate-card-row'>
                ...
                <div>
                  ...
                  {affUser.total_earned > 0 ? affUser.total_earned : <>0</>}{' '}
                  Tokens
                </div>
                <div>
                  <strong>Last Commission Earned:</strong>{' '}
                  {affUser.last_earned_at ? (
                    affUser.last_earned_at.substring(
                      0,
                      affUser.last_earned_at.indexOf('T')
                    )
                  ) : (
                    <>Start earning today!</>
                  )}
