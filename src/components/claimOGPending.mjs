...
export default function ClaimOGPending() {
    ...
    const contractClaim = async () => await sdk.getContractFromAbi(
        ...
        const divCall = await contract.call("getAccountDividendsInfo", address).then((divInfo)=>{
  ...
  return (
    ...
    {(!isClaiming)?(
      <div>
        <span className='yellow'><strong>BUSD REWARDS</strong></span><br></br>
        Claimable Amount: {claimableAmount} BUSD<br></br>
        Total Rewards Claimed: {totalClaimed} BUSD<br></br>
        
        {(address && ((secondsUntilClaim === 0) || ({claimableInTime} >= Date.now())))?(
            <div><button className="claim-btn" onClick={()=>handleClaim()}>Claim Pending Rewards</button></div>
            ):(
            <div className='claim-countdown'>Next Claim Available:<br></br>{(claimableInTime)?(getCountdown(claimableInTime)):(<>Unavailable</>)}</div> 
            )} 
