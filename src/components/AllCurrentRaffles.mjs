...
export default function AllCurrentRaffles(){
    ...
    <div className='half-page-menu-wrapper'>
        ...
        <div className='half-page-card-menu-div'>
            <div className="feature-overview-div1" style={{margin: '5px auto', width: '100%', textAlign: 'center'}}>
                <div className='flex-row-no-margin'>
                    <div>
                        <div><img className="wallet-casino-profile-img" src={user[4]} alt="Scrooge Casino profile picture" /></div>
                    </div>

                    <div>
                        <div className='inlineTitle' style={{fontSize: '20px'}}>RAFFLE TICKET BALANCE</div>
                        
                        <div>YOU HAVE <span className='green' style={{fontSize: '26px', fontWeight: 'bold'}}>{userRaffleTickets} TICKETS</span> TO SPEND</div>
                    </div>
                </div>
            </div>
            <div className="feature-overview-div3" style={{margin: '5px auto', width: '100%', textAlign: 'left'}}>
                <p>Scrooge Raffles are the perfect way to finally get that incredible prize you've had your eye on but couldn't afford.</p>
                <p> Simply purchase a Scrooge Entry NFT from below and you will automatically recieve your NFT, and your Scrooge Raffles account will be credited with your bonus raffle entry tickets.
                </p>  
            </div>
            <div className="feature-overview-div3" style={{margin: '5px auto', width: '100%', textAlign: 'center'}}>
                <div style={{fontSize: '20px'}}>Winning big with Scrooge Raffles is as easy as 1, 2, 3!</div>
                <div className='flex-row-no-margin' style={{marginTop: '25px'}}>
                    ...
                    <div className='feature-overview-block bg-animated2' style={{fontWeight: 'bold'}}>3. WIN BIG!</div>
                </div>
            </div>
        </div>
        ...
        {(raffles.length !== 0)?(<>
            <div className='leaderboard-card-div' style={{margin: '0 auto'}}>
                <div className='pulse'><strong>ENTER TO WIN!</strong></div>
                <div className='teeter'><div className='ending-soon bg-animated3'>ENDING SOON</div></div>
                <div className="pageTitle" style={{marginBottom: "-20px"}}>
                    <h1>{raffles[0].prize_details.name}</h1>
                </div>
                <div className='flex-row-no-margin'>
                    <div className='raffle-data-box'>
                        <div><strong>ENTRY FEE:</strong></div>
                        <div>{raffles[0].entry_fee} Tickets</div>
                    </div>
                    <div><img className="raffle-img pulse" src={raffles[0].prize_details.image_url} alt="Scrooge raffle prize" /></div>
                    <div className='raffle-data-box'>
                        <div><strong>DRAW:</strong></div>
                        <div><Countdown date={raffles[0].end_date}></Countdown></div>
                    </div>
                </div>
                ...
                <div className='leaderboard-row'>
                    <div>
                        <div><strong># OF PRIZES</strong></div>
                        <div>{raffles[0].prize_qty}</div>
                    </div>
                    <div>
                        <div><strong># OF ENTRIES</strong></div>
                        <div>{raffles[0].entries_count}</div>
                    </div>
                    <div>
                        <div><strong>TOTAL VALUE</strong></div>
                        <div>{(raffles[0].prize_details.price * raffles[0].prize_qty).toLocaleString('en-US')} Tokens</div>
                    </div>
                </div>
                <button className='button4' style={{width: '60%', margin: '35px auto 0 auto'}} onClick={()=>enterRaffle(raffles[0]._id)}>ENTER RAFFLE</button>
            </div>
        </>):(<></>)}
        ...
        <div className="pageTitle" style={{marginBottom: '0px'}}>
            ...
            {items.slice(0, 4).map((item, index) => (<>
                <div className='raffles-header-shop-card' key={item+index}>
                    <div>{item.name}</div>
                    <div><img className="card-img" src={item.image_url} alt="purchase scrooge raffle NFTs" /></div>
                    <div className='green' style={{fontWeight: 600, fontSize: '18px'}}>SPECIAL BONUS:<br></br> {item.chip_value} FREE RAFFLE ENTR{(item.chip_value>1)?(<span>IES</span>):(<span>Y</span>)}</div>
                    <div style={{marginTop: '15px'}}>PRICE: {((parseInt(item.price.$numberDecimal)/currentPriceOG).toFixed(0))} SCROOGE</div>
                    <div style={{fontSize: '12px'}}>(~ ${parseInt(item.price.$numberDecimal)} USD)</div>
                    <div style={{marginTop: '15px'}}><button onClick={()=>handlePurchase(parseInt(item.price.$numberDecimal), item._id)} className='button3 pulse'>Purchase</button></div>
                </div>
    ...
    {(raffles.length !== 0)?(<>
        ...
        <div className="min-menu-div" style={{marginTop: '100px'}}>
            <div>FILTER BY:</div>
            <button className="min-menu-btn" onClick={()=>{alert(true)}}>BADGES</button>
            <button className="min-menu-btn" onClick={()=>{alert(true)}}>CRYPTO</button>
            <button className="min-menu-btn" onClick={()=>{alert(true)}}>MERCH</button>
            <button className="min-menu-btn" onClick={()=>{alert(true)}}>NFTS</button>
        </div>
            ...
            <div className="flex-row-no-margin">
                {raffles.map((raffle) => (
                    <div className='leaderboard-card-div' key={raffle._id}>
                        <div className='teeter'><strong>ENTER TO WIN!</strong></div>
                        ...
                        <div className='flex-row-no-margin'>
                            ...
                            <div><img className="raffle-img pulse" src={raffle.prize_details.image_url} alt="Scrooge raffle prize" /></div>
                            <div className='raffle-data-box'>
                                <div><strong>DRAW:</strong></div>
                                <div><Countdown date={raffle.end_date}></Countdown></div>
                            </div>
                        </div>
                        <div style={{marginBottom: "25px"}}>
                        ...
                        <div className='leaderboard-row'>
                            ...
                            <div>
                                <div><strong>TOTAL VALUE</strong></div>
                                <div>{(raffle.prize_details.price * raffle.prize_qty).toLocaleString('en-US')} Tokens</div>
                            </div>
                        </div>
                        <button className='button4' style={{width: '60%', margin: '35px auto 0 auto'}} onClick={()=>enterRaffle(raffle._id)}>ENTER RAFFLE</button>
                        <div className='fine-print-txt' style={{marginTop: '15px', color: 'darkgray'}}>Raffle ID: {raffle._id}</div>
                    </div>
                ))}
            </div>
        </div>
